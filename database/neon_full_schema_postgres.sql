-- ============================================================================
-- MANUS FISIO - SCRIPT COMPLETO DE CRIAÇÃO DO BANCO NEON (POSTGRESQL)
-- ============================================================================
-- Uso recomendado:
--   1. Abra Neon Console > SQL Editor.
...
--   - Para ambiente local com ferramentas SQL padrão, prefira migrations versionadas.
-- ============================================================================
...
BEGIN
  ...
  RETURN EXISTS (
    ...
    WHERE nc.notebook_id = notebook_uuid
      AND nc.user_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid
      AND (
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE id = project_uuid
      AND (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR owner_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid)
  ) THEN
    RETURN TRUE;
  END IF;
  ...
  RETURN EXISTS (
    ...
    WHERE pc.project_id = project_uuid
      AND pc.user_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid
      AND (
        permission_level = 'read'
...
CREATE POLICY "Users can view their own profile" ON public.users
  FOR SELECT USING (id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid);

DROP POLICY IF EXISTS "Users can update their own profile" ON public.users;
CREATE POLICY "Users can update their own profile" ON public.users
  FOR UPDATE USING (id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid);
...
CREATE POLICY "Users can view notebooks they have access to" ON public.notebooks
  FOR SELECT USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_notebook_permission(id, 'read'));
...
CREATE POLICY "Users can create notebooks" ON public.notebooks
  FOR INSERT WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (created_by IS NULL OR created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid));
...
CREATE POLICY "Users can update notebooks they can write" ON public.notebooks
  FOR UPDATE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_notebook_permission(id, 'write'));
...
CREATE POLICY "Users can delete notebooks they administer" ON public.notebooks
  FOR DELETE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_notebook_permission(id, 'admin'));
...
CREATE POLICY "Users can view projects they have access to" ON public.projects
  FOR SELECT USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR owner_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_project_permission(id, 'read'));
...
CREATE POLICY "Users can update projects they can write" ON public.projects
  FOR UPDATE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR owner_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_project_permission(id, 'write'));
...
CREATE POLICY "Users can delete projects they administer" ON public.projects
  FOR DELETE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR owner_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_project_permission(id, 'admin'));
...
CREATE POLICY "Users can view project tasks" ON public.tasks
  FOR SELECT USING (
    created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid
    OR assigned_to = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid
    OR assignee_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid
...
CREATE POLICY "Users can create project tasks" ON public.tasks
  FOR INSERT WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (created_by IS NULL OR created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_project_permission(project_id, 'write')));
...
CREATE POLICY "Users can update project tasks" ON public.tasks
  FOR UPDATE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR assigned_to = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR assignee_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_project_permission(project_id, 'write'));
...
CREATE POLICY "Users can delete project tasks" ON public.tasks
  FOR DELETE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.has_project_permission(project_id, 'admin'));
...
CREATE POLICY "Authenticated users can comment" ON public.comments
  FOR ALL USING (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL)
  WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (author_id IS NULL OR author_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid));

DROP POLICY IF EXISTS "Users can view own activity logs" ON public.activity_logs;
CREATE POLICY "Users can view own activity logs" ON public.activity_logs
  FOR SELECT USING (user_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_admin());

DROP POLICY IF EXISTS "System can insert activity logs" ON public.activity_logs;
CREATE POLICY "System can insert activity logs" ON public.activity_logs
  FOR INSERT WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL);
...
CREATE POLICY "Clinicians can view patients" ON public.patients
  FOR SELECT USING (public.is_mentor() OR created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid);
...
CREATE POLICY "Clinicians can create patients" ON public.patients
  FOR INSERT WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (created_by IS NULL OR created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_mentor()));
...
CREATE POLICY "Clinicians can update patients" ON public.patients
  FOR UPDATE USING (public.is_mentor() OR created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid);
...
CREATE POLICY "Clinicians can manage patient records" ON public.patient_records
  FOR ALL USING (public.is_mentor() OR created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid)
  WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (created_by IS NULL OR created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_mentor()));
...
CREATE POLICY "Users can view own calendar events" ON public.calendar_events
  FOR SELECT USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR nullif(current_setting('request.jwt.claim.sub', true), '')::uuid = ANY(attendees) OR public.is_admin());
...
CREATE POLICY "Users can update own calendar events" ON public.calendar_events
  FOR UPDATE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_admin());
...
CREATE POLICY "Users can delete own calendar events" ON public.calendar_events
  FOR DELETE USING (created_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_admin());
...
CREATE POLICY "Users can manage own notification settings" ON public.notification_settings
  FOR ALL USING (user_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid) WITH CHECK (user_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid);
...
CREATE POLICY "Clinicians can manage physiotherapy evaluations" ON public.physiotherapy_evaluations
  FOR ALL USING (public.is_mentor() OR evaluator_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid)
  WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (evaluator_id IS NULL OR evaluator_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_mentor()));
...
CREATE POLICY "Clinicians can manage treatment sessions" ON public.treatment_sessions
  FOR ALL USING (public.is_mentor() OR therapist_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid)
  WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (therapist_id IS NULL OR therapist_id = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_mentor()));
...
CREATE POLICY "Authenticated users can view active exercises" ON public.exercise_library
  FOR SELECT USING (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND is_active = true);
...
CREATE POLICY "Clinicians can manage exercise prescriptions" ON public.exercise_prescriptions
  FOR ALL USING (public.is_mentor() OR prescribed_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid)
  WITH CHECK (nullif(current_setting('request.jwt.claim.sub', true), '')::uuid IS NOT NULL AND (prescribed_by IS NULL OR prescribed_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid OR public.is_mentor()));
...
CREATE POLICY "Clinicians can manage prescription exercises" ON public.prescription_exercises
  FOR ALL USING (
    ...
    OR EXISTS (
      ...
      WHERE ep.id = prescription_id AND ep.prescribed_by = nullif(current_setting('request.jwt.claim.sub', true), '')::uuid
    )
  )
  WITH CHECK (
    public.is_mentor()
